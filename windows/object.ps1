...
function newRemoteWin {
  ...
  $remoteWin | add-member -MemberType ScriptMethod -Value { ##joinDomain
    param($domainName, $domainUser, $domainPassword, $dnsIp)
    if($dnsIp) {
      $cmd = "
      ...
      "
      $this.executePsTxtRemote($cmd, "set DNS server IP")
    }
    $cmd = "
    ...
    "
    $this.executePsTxtRemote($cmd, "join machine to domain")
  } -name joinDomain
  ...
  $remoteWin | add-member -MemberType ScriptMethod -Value { ##windowsUpdate
    ...
    $cmd = {
      ...
      $wc.downloadfile('http://bit.ly/1P3hdiz', 'c:\temp\updateWindows2.ps1')
    }
    invoke-command -scriptblock $cmd -session $this.session | out-null
  ...
  $remoteWin | add-member -MemberType ScriptMethod -Value { ##windowsUpdateSync
    ...
    $cmd = "
      ...
      `$wc = new-object system.net.webclient
      `$wc.downloadfile('http://bit.ly/1NXR8Bq', 'c:\temp\wu.ps1')
    "
